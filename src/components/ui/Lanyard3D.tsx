...
export default function Lanyard3D({
  ...
  const onDragEnd = useCallback(
    (_e: unknown, info: { velocity: { x: number; y: number } }) => {
      ...
      animate(dragX, flingX, {
        duration: 0.08, ease: "easeOut", onComplete: () =>
          animate(dragX, 0, SNAP_SPRING)
      });
      animate(dragY, flingY, {
        duration: 0.08, ease: "easeOut", onComplete: () =>
          ...
          >
            ...
            <div className="px-5 py-3 grid grid-cols-2 gap-2">
              ...
              <div className="bg-white/5 rounded-lg px-3 py-2">
                ...
                <div className="text-[11px] font-semibold" style={{ color: "#22c55e" }}>In Internship Program</div>
              </div>
            </div>
